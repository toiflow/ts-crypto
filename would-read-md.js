#!/usr/bin/env node
// would-read-md.js — fetch crypto news from RSS, print news context to stdout

const RSS_URLS = [
  'https://news.google.com/rss/search?q=bitcoin+ethereum+crypto+market+price&hl=en&gl=US&ceid=US:en',
  'https://news.google.com/rss/search?q=cryptocurrency+DeFi+blockchain+regulation&hl=en&gl=US&ceid=US:en'
];

const KEYWORDS = ['bitcoin', 'ethereum', 'crypto', 'btc', 'eth', 'blockchain', 'defi', 'altcoin', 'web3', 'coinbase', 'binance', 'solana'];
...
async function main() {
  ...
  for (const url of RSS_URLS) {
    try {
      const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; gs-crypto/1.0)' } });
